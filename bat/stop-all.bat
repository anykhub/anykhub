...
powershell -NoProfile -Command "Get-CimInstance Win32_Process -Filter \"Name='cmd.exe' AND CommandLine LIKE '%%Redis%%'\" | ForEach-Object { Stop-Process -Id `$_.ProcessId -Force -ErrorAction SilentlyContinue }" >nul 2>&1
...
:: Use PowerShell to find the java.exe process with this JAR,
:: get its ParentProcessId (which is the cmd.exe window),
:: and kill both the java process and its parent cmd process.
powershell -NoProfile -Command "$jv=Get-CimInstance Win32_Process -Filter \"Name='java.exe' AND CommandLine LIKE '%%java%%-jar%%%JAR%%%'\" ; if($jv) { foreach($p in $jv){ try{ Stop-Process -Id $p.ParentProcessId -Force -ErrorAction SilentlyContinue }catch{}; try{ Stop-Process -Id $p.ProcessId -Force -ErrorAction SilentlyContinue }catch{} } }" >nul 2>&1

:: Fallback fallback just in case
